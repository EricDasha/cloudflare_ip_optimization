...
- `cf.090227.xyz`：第三方优选目录；同时抓取 `/ct?ips=6`、`/cu`、`/cmcc?ips=8` API，并解析目录列出的固定优选域名 DNS。
...
启用固定转发 IP 池后，CFnat 会按新连接轮询池中的 IP。`CFNAT_NUM` 表示单个连接并发尝试的连续轮询目标数；设为 `1` 时每个连接只使用一个 IP，设为更大值时会优先连接其中最快的可达 IP。
